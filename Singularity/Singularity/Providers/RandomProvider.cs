...
namespace Singularity
{
	...
	///  can each be represented by a single seed value (int) when using FRandom.
	///  
	///  Notes.
	...
	public class RandomProvider
	{
		...
		private const Double RealUnitUint = 1.0/((Double)UInt32.MaxValue+1.0);
		private const UInt32 Y=842502087;
		private const UInt32 Z=3579807591;
		private const UInt32 W=273326509;
		...
		public RandomProvider()
		{
			...
			Reinitialise((Int32)Environment.TickCount);
		}
		...
		/// System.Random
		/// </summary>
		public RandomProvider(Int32 seed)
		{
			Reinitialise(seed);
		}
		...
		public Int32 Next(Int32 upperBound)
		{
			...
			return (Int32)((RealUnitInt*(Int32)(0x7FFFFFFF&(_w=(_w^(_w>>19))^(t^(t>>8)))))*upperBound);
		}
		...
		public Int32 Next(Int32 lowerBound, Int32 upperBound)
		{
			...
			{	// If range is <0 then an overflow has occured and must resort to using long integer arithmetic instead (slower).
				...
				return lowerBound+(Int32)((RealUnitUint*(Double)(_w=(_w^(_w>>19))^(t^(t>>8))))*(Double)((Int64)upperBound-(Int64)lowerBound));
			}
				
			// 31 bits of precision will suffice if range<=int.MaxValue. This allows us to cast to an int and gain
			// a little more performance.
			return lowerBound+(Int32)((RealUnitInt*(Double)(Int32)(0x7FFFFFFF&(_w=(_w^(_w>>19))^(t^(t>>8)))))*(Double)range);
		}
		...
		{	
			...
			// System.Random.
			return (RealUnitInt*(Int32)(0x7FFFFFFF&(_w=(_w^(_w>>19))^(t^(t>>8)))));			
		}
